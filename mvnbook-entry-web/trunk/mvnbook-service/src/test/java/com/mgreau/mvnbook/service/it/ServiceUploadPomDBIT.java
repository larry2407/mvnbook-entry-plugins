package com.mgreau.mvnbook.service.it;
 ...
 *
 */
public class ServiceUploadPomDBIT {
	...
	public void testLoadPomAndSave() {
		...
		Plugin pJar = null;
		try {
			pHelp = service.loadByPomFile("maven-help-plugin-2.1.1.pom", category, Type.REPORTING);
			pJar = service.loadByPomFile("maven-jar-plugin-2.3.1.pom", category, Type.BUILD);
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
